...
const RouteBuilder = (customRouteContext) => {
  ...
  const handlePreviousPageAnimation = () => {
    ...
    const brightness = wx.skyRoutePageMask == 0 ? 1 : (1 + (prevPageBrightness - 1) * lt) 
    let scale = 1
...
function rightRouteBuilder() { 
  wx.router.addRouteBuilder('rightRoute',RouteBuilder)
}
export default rightRouteBuilder
